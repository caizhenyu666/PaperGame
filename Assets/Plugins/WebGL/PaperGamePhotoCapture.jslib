mergeInto(LibraryManager.library, {
  PaperGame_OpenPhotoCapture: function (receiverNamePointer) {
    ...
    css.textContent = [
      ...
      '#pg-cam-frame{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:min(560px,calc(100% - 176px));aspect-ratio:8/5;border:2.5px solid rgba(255,255,255,.88);border-radius:14px;box-shadow:0 0 0 9999px rgba(0,0,0,.42);pointer-events:none}',
    ...
    btn.addEventListener('click', function () {
      ...
      var vScale = Math.max(viewWidth / video.videoWidth, viewHeight / video.videoHeight);
